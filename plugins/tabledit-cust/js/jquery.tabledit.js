 ...
 * @version 1.2.3
 * @author Celso Marques Gavin(modified)
 *
 */
...
(function($) {
    ...
    $.fn.Tabledit = function(options) {
        ...
        {
            var serialize = $table.find('.tabledit-input').serializeArray();

            if (!serialize) {
                return false;
            }

            var request = settings.onAjax(action, serialize);
            ...
            var jqXHR = $.ajax({
              ...
              success : function(data, textStatus, jqXHR) {
                ...
                var result = settings.onSuccess(data, textStatus, jqXHR);

                if (result && action === settings.buttons.edit.action) {
